...
<body> <!-- 연락내역 상세페이지 -->
    ...
    <input type="hidden" name="no" value="${result.no}"/>
        ...
        <div class="card-footer d-flex justify-content-center">
            <button type ="button" class="btn btn-secondary mx-2" onclick="window.history.back()">뒤로가기</button> <!-- 아래 url요청 -> get -->
            <button type="button" class="btn btn-primary mx-2" onclick="location.href='/form/answerForm.do?no=${result.no}'">답변</button> 
            <button type="submit" class="btn btn-danger mx-2">삭제</button>									<!-- 답변 클릭시 답변창으로 이동 // no를 받고있으니 쿼리문 사용!-->
        </div>
        </form>
    </div>
</section>
