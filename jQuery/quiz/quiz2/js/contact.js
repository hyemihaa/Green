...
//  이름
function checkName() {
    const nameInput =("#name").val(); //이름
    const resultName =("#result-name"); // 이름 값 출력할 곳
    
    const regex1= /^[ㄱ-ㅎ|가-힣]/; // 한글만
    
    // 이름규칙 
    if(!(regex1.test(nameInput))) {
        resultName.html("한글만 사용 가능합니다.");
        resultName.css("color", "red");
    }
    else {
        resultName.html("이름은 한글만 가능합니다.");
    }
}
...
function checkEmail() {
    ...
    if(emailPattern.test(emailInput)) {
        resultEmail.html("사용 가능한 이메일 입니다.");
        resultEmail.css("color", "green");
    }
    else {
        resultEmail.html("사용 불가능한 이메일 입니다.");
...
function checkMessage() {
   // <textarea id="message" name="message" onkeyup="checkMessage()" rows="4" required></textarea> html 요소
    const messageText = ("#message"); // 사용자 입력값
    const counter = ("#message-counter");   //p태그
    
    // 0/500
    counter.html(messageText.val().length + "/ 500");
    if(messageText.val().length > 500)
    messageText.val(messageText.val().substring(0, 500));
...
function getContact3(){
    const nameInput = ("#name").val(); //이름
    const emailInput = ("#email").val(); //이메일
